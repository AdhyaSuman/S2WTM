...
class ETM(BaseETM):
    ...
    def inference(self):
        ...
        for idx, ind in enumerate(indices):
            if ind.shape[0] == self.hyperparameters['batch_size']:  # Check batch size
                data_batch = data.get_batch(self.test_tokens, self.test_counts,
                                            ind, len(self.vocab.keys()),
                                            self.device) # Only process if it is the correct batch size
            else:
                print(f"Skipping last batch of size {ind.shape[0]}. Expected: {self.hyperparameters['batch_size']}")
                break # or continue if you want to process subsequent batches (if any)
            sums = data_batch.sum(1).unsqueeze(1)
            if self.hyperparameters['bow_norm']:
                normalized_data_batch = data_batch / sums
            else:
